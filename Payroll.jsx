...
function PieceWork({ period, workers, addPiece, piecework, columns }) {
  ...
  // Live earning preview — units × rate (uses the worker's default rate
  // for the chosen work type when the rate field is left blank).
  const previewWorker = workers.find((w) => w.id === form.workerId)
  const previewRate = Number(form.rate) || defaultRate(previewWorker, form.workType) || 0
  const previewUnits = Number(form.units) || 0
  const previewEarning = Math.round(previewUnits * previewRate * 100) / 100
  ...
  return (
    <div className="space-y-4">
      <Card>
        ...
        {/* Live earning preview — auto = units × rate (visible while typing). */}
        <div className="mt-3 flex justify-end text-sm num text-[var(--muted)] border-t border-dashed border-[var(--border)] pt-2">
          {fmtNum(previewUnits)} × {fmtCurrency(previewRate)}{' '}
          <span className="font-bold text-[var(--text)] ms-2">
            = {fmtCurrency(previewEarning)}
          </span>
        </div>
      </Card>
      <Card>
