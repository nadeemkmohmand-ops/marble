...
export default function Table({ columns, rows }) {
  return (
    /* no overflow-hidden — Nastaliq Urdu glyphs would get clipped top/bottom */
        ...
        <table className="w-full min-w-[640px] text-sm">
            ...
            <tr className="border-b border-border bg-secondary/70 text-xs text-muted dark:border-gray-700 dark:bg-gray-700/40">
              {columns.map((col) => (
                <th
                  key={col.key}
                  scope="col"
                  className={`urdu-text whitespace-nowrap px-4 py-3 text-start font-semibold ${col.className || ''}`}
                >
                  {col.header}
                </th>
              ))}
          ...
          <tbody className="divide-y divide-border dark:divide-gray-700">
            {rows.map((row, rowIndex) => (
              ...
              >
                {columns.map((col) => (
                  <td key={col.key} className={`urdu-text whitespace-nowrap px-4 py-3 text-main ${col.className || ''}`}>
